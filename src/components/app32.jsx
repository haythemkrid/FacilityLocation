...
function App3Capacitated() {
    ...
    const handleCapacitiesChange = (index, value) => {
        const updatedCapacities = [...capacities];
        updatedCapacities[index] = parseFloat(value) || 0;
        setCapacities(updatedCapacities);
    };

    const handleDemandsChange = (index, value) => {
        const updatedDemands = [...demands];
        updatedDemands[index] = parseFloat(value) || 0;
    ...
    const handleNumFacilitiesChange = (value) => {
        ...
        // Resize fixed_costs array
        const updatedFixedCosts = Array(newNumFacilities).fill(0);
        setFixedCosts(updatedFixedCosts);

        // Resize transportation_costs matrix
        const updatedTransportationCosts = Array.from({ length: num_customers }, (_, rowIndex) =>
            transportation_costs[rowIndex]
                ? transportation_costs[rowIndex].slice(0, newNumFacilities).concat(
                    Array(Math.max(0, newNumFacilities - transportation_costs[rowIndex].length)).fill(0)
                )
                : Array(newNumFacilities).fill(0)
        );
        setTransportationCosts(updatedTransportationCosts);

        // Resize capacities array
        const updatedCapacities = Array(newNumFacilities).fill(0);
        setCapacities(updatedCapacities);
    };
    ...
    const handleNumCustomersChange = (value) => {
        ...
        const updatedDemands = Array(newNumCustomers).fill(0);
        setDemands(updatedDemands);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (
            ...
            capacities.length !== num_facilities ||
            demands.length !== num_customers ||
            transportation_costs.length !== num_customers ||
            !transportation_costs.every((row) => row.length === num_facilities)
        ) {
            setError("Invalid input data. Ensure all costs, capacities, and dimensions are correct.");
            return;
        }

        try {
            const response = await axios.post("http://localhost:5000/app3/capacitated", {
                num_facilities,
                num_customers,
                fixed_costs,
        ...
        <>
            <div className="App bg-gray-100 min-h-screen p-10">
                ...
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md max-w-xl mx-auto">
                    ...
                    <div>
                        <label className="block font-medium text-gray-700 mb-3">Capacities (Facilities):</label>
                        {Array.from({length: num_facilities}, (_, index) => (
                            <div key={index} className="mb-2">
                                <input
                                    type="number"
                                    placeholder={`Capacity Facility ${index + 1}`}
                                    value={capacities[index] || ""}
                                    onChange={(e) => handleCapacitiesChange(index, e.target.value)}
                                    className="w-full p-2 border rounded"
                                />
                            </div>
                        ))}
                    ...
                    <div>
                        <label className="block font-medium text-gray-700 mb-3">Demands (Customers):</label>
                        {Array.from({length: num_customers}, (_, index) => (
                            <div key={index} className="mb-2">
                                <input
                                    type="number"
                                    placeholder={`Demand Customer ${index + 1}`}
                                    value={demands[index] || ""}
                                    onChange={(e) => handleDemandsChange(index, e.target.value)}
                                    className="w-full p-2 border rounded"
                                />
                            </div>
                        ))}
                ...
                {output && (
                    <div className="mt-8 bg-white p-6 rounded shadow-md max-w-xl mx-auto">
                        ...
                        {/* Capacities Used */}
                        <div className="mt-4">
                            <h3 className="font-medium">Capacities Used by Each Facility:</h3>
                            <ul className="list-disc pl-6 mt-2 text-gray-700">
                                {output[1].map((capacity, index) => (
...
export default App3Capacitated;