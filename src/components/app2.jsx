...
function App2() {
    ...
    const [num_to_open, setNumToOpen] = useState(1);
    ...
    const handleNumToOpenChange = (value) => {
        setNumToOpen(Math.max(1, parseInt(value) || 1));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!num_to_open || num_to_open > num_installations) {
            setError("Number of installations to open must be between 1 and the total installations.");
            return;
        }

        try {
            const response = await axios.post("http://localhost:5000/app2", {
                num_installations,
                num_demands,
                distances,
                service_levels,
        ...
        <>
            ...
            <div className="App bg-gray-100 min-h-screen p-10">
                <h1 className="text-2xl font-bold text-center text-blue-600 mb-6">
                    Max Covering Problem with Distance Constraints
                </h1>
                <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md max-w-xl mx-auto">
                    ...
                    <div className="mb-4">
                        <label className="block font-medium text-gray-700 mb-2">
                            Number of Installations to Open:
                        </label>
                        <input
                            type="number"
                            value={num_to_open}
                            onChange={(e) => handleNumToOpenChange(e.target.value)}
                            className="w-full p-2 border rounded"
                            min="1"
                            max={num_installations}
                        />
                    </div>
                    <div>
